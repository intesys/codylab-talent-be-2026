...
public class OpenApiActivityController implements ActivityControllerApi {
    ...
    public ResponseEntity<ActivityApiDTO> getActivityById(Long activityId) {
        ...
        if (activityOptional.isPresent()) {
            ...
            problemApiDTO.setTitle("Activity not found");
            problemApiDTO.setDetail("Activity with ID " + activityId + " not found");
            problemApiDTO.setStatus(404);
            problemApiDTO.setInstance(URI.create("/activities/" + activityId));
    ...
    public ResponseEntity<ActivityApiDTO> createActivity(ActivityApiDTO activityApiDTO) {
        ...
        return ResponseEntity.created(URI.create("/activities/" + activityId))
                .body(activityMapper.mapToApiDto(activityService.findById(activityId).get()));
    }
